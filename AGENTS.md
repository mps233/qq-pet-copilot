...
| `dashboard.py` | 手机仪表盘（纯标准库 HTTP 服务，默认端口 8787，`./dashboard.sh` 后台常驻；日志 3s、数据 6s 自动刷新）。**改 UI 前必读 `qqpet_assets/web/INTEGRATION.md`**（结构/ID 契约、图标路由、验证方式、踩坑记录）+ `UI_SPEC_MEASURED.md`/`UI_TREE_OFFICIAL.md`（官方实测尺寸）。验证：`python3 dashboard.py --port 8899` 起副本（不影响 8787 在跑的实例）。**截图必须走 iframe 承载**：macOS headless Chrome 最小视口 500px，`--window-size=390` 截出的 PNG 宽 390 但页面仍按 500 排版后裁右侧，直接看会误判成"窄屏布局溢出"（见 INTEGRATION.md 5.1）。页面元素位置可自由重排，但 `main > [data-page]` 直系与 `#tabbar button[data-tab]` 两个 JS 契约不能动；**滚动架构别改回去**：`html` 是 `overflow:hidden` 只当画布（文档层不可滚，否则 iOS PWA 整页橡皮筋回弹——`overscroll-behavior` 管不到根滚动器），真正滚动的是 `body`（`height:100%` + `overflow-y:auto` + `overscroll-behavior:none`），内层滚动区（`#logbox`/`.advlist`）同样用 `none`（`contain` 只挡链式传递、回弹照旧）。**历史栈模型（侧滑返回）**：`navDepth` = 导航层级（总览 0 / 内页 1 / 设置二级 2），栈与层级一一对应 —— 往下一层 `pushState`、同级互切 `replaceState`、往上一层 `history.back()`、跨层回总览 `go(-navDepth)`；**"返回"类操作绝不能用 `pushState`**（按钮在压栈、手势在退栈，方向相反 → 侧滑要一路退回访问轨迹，实测 6 页路径要滑 5 次；且侧滑回到"设置一级"时页面名不变，层级同步必须写在 `showTab` 的 `if(name===curTab) return` **之前**，否则会多退一层甚至退出应用，见 INTEGRATION.md 第十二轮）。**改完 `dashboard.py` 必须重启 8787**（`HTML` 是模块级常量，只在 8899 起副本验证 ≠ 线上已更新） |
...
- **学园选课（课时时长）**：每阶段 6 张课卡 = 3×10分钟 + 3×30分钟（两组内顺序都是
  力量/智力/魅力；初级卡名：体能/看图识世界/闪亮自我表达 与 田径运动/世界地理/演说表达，
  另有第 7 张"萌芽夏令营"未测）；10分钟课单位收益更高（同 30 分钟消耗：10分课 +6属性/
  +30学分 vs 30分课 +5/+25），默认 10分钟——`school.duration` 可选、仪表盘「学习」区可改。
  30分钟课选课 = 小步慢拖扫描卡名（**快拖惯性大、点选会点空——踩过坑**）+ 点后按详情
  "奖励<属性>+N"核对（10分+2 / 30分+5，OCR 会把"+5点"读成"+50"、正则取第一位数字），
  不通过归位重试一次；未知阶段名字表（`COURSE30_NAMES`）待毕业实测补。
...
- **被雇佣检查调度**：`employed.enabled` 开启时，被雇佣时间段（`employed.time_range`，
  ...
  **非阻塞**：`employed_recall_ready` 单次判定到召回时机（`employed.action` 策略）
  ...
  - **学习/工作时长规则（替代旧“每日点数”）**：学习结算按持久化的**课时时长**累计
    （`school_progress.json` 的 `duration`：10分钟/30分钟，学习开始时
    `set_current_school_duration` 写入；旧会话无该字段时回退学园阶段映射
    初级10/中级20/高级30/进修45 分钟）；打工结算按持久化的 `work.duration` 累计
