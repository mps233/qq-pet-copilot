...
TASK_SETTING_FIELDS = [
    ...
    ('school.duration', '课时档位（短课=卡1-3 / 长课=卡4-6；各学院分钟数不同，实际时长自动读取）',
     ['短课', '长课']),
    ('school.times_per_day', '每天学习次数（0 不限）', 'int'),
    ('schedule.study_quota_hours', '今日学习配额（小时，0=不学习）', 'int'),
    ('schedule.work_quota_hours', '今日打工配额（小时，0=不打工）', 'int'),
    ('schedule.daily_hour_limit', '学习停止（学习+打工合计小时，0 不限）', 'int'),
    ('schedule.work_stop_hours', '打工停止（学习+打工合计小时，0 不限）', 'int'),
    ('schedule.efficiency_tier1_hours', '效率降 25% 门槛（合计小时）', 'int'),
    ('schedule.efficiency_tier2_hours', '效率降 10% 门槛（合计小时）', 'int'),
    ('career.watch', '隐藏职业解锁哨兵（每节课后查职业树）', 'bool'),
    ('career.stop_study_on_unlock', '解锁后自动停止学习', 'bool'),
    ('career.check_interval_min', '哨兵兜底检查间隔（分钟，0=只每节课后）', 'int'),
    ...
    ('hire_friend.friend_name', '雇佣好友名称（多个逗号分隔，按序取第一个能找到的）', 'str'),
    ...
    ('employed.action', '被雇佣后处理', ['等到25/75（小于45min）', '等到25/75', '立刻召回', '让利雇主（不召回）']),
