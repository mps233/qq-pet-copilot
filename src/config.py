...
class PkConfig:
    ...
    start_time: str = "00:01"
    # PK 目标过滤：只打这些好友（玩家昵称或宠物名，逗号分隔，部分匹配）；空 = 不限
    only_names: str = ""
    # PK 目标过滤：跳过这些好友（玩家昵称或宠物名，逗号分隔，部分匹配）；空 = 不跳过
    skip_names: str = ""
    # PK 目标过滤：只打等级 ≤ N 的好友宠物（0 = 不限；-1 = 只打等级比自己低的，没有可打的时兜底打任意等级）
    max_level: int = 0
...
class EmployedConfig:
    ...
    # 立刻召回（进被雇佣面板直接点"现在召回"）/
    # 让利雇主（不召回）（小号工具人：主动不召回，宠物留在雇主那儿继续打工、收益归雇主方）
